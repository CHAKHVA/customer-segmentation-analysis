...
def detect_outliers_iqr(df: pd.DataFrame, column: str) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    """
...
def handle_outliers(df: pd.DataFrame, column: str, method: str = 'keep') -> pd.DataFrame:
    """
    ...
    if method == 'keep':
        ...
        df_clean.loc[df_clean[column] < stats['lower_bound'], column] = stats['lower_bound']
        df_clean.loc[df_clean[column] > stats['upper_bound'], column] = stats['upper_bound']
...
def create_income_categories(df: pd.DataFrame) -> pd.DataFrame:
    ...
    labels = ['Low Income (<40k)', 'Medium Income (40-70k)',
              'High Income (70-100k)', 'Very High Income (>100k)']

    df_copy['Income_Category'] = pd.cut(df_copy['Annual Income (k$)'],
...
def create_spending_categories(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_copy['Spending_Category'] = pd.cut(df_copy['Spending Score (1-100)'],
                                           bins=bins, labels=labels, include_lowest=True)
...
def generate_preprocessing_report(df_original: pd.DataFrame,
